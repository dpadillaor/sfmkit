...
class TestSolve:
    def test_recovers_perturbed_ground_truth(self, scene):
    ...
    def test_robust_loss_resists_outliers(self, scene):
        ...
        plain = solve_bundle(scene.K, images, poses, noisy_points, corrupted, loss="linear")

        err = lambda r: max(rotation_error_deg(r.poses[n].R, poses[n].R) for n in images)  # noqa: E731
        assert err(robust) <= err(plain) + 1e-9

    def test_reports_timing_and_iterations(self, scene):
        images, poses, points, obs = _problem_from_scene(scene, n_cameras=3)
        res = solve_bundle(scene.K, images, poses, points + 0.01, obs)
