...
allm = [io.load_matches(f) for f in sorted((run / "verify").glob("*.npz"))]
allm = [m for m in allm if cfg.localize.query not in (m.image0, m.image1)]
star = [m for m in allm if ref in (m.image0, m.image1)]

out = {}
for label, ms in (("star", star), ("complete", allm)):
    tracks = build_tracks(ms, min_length=cfg.sfm.min_track_length)
    ...
    summary = {k: v for k, v in out[label].items() if k != "per_camera"}
    print(f"{label}: {json.dumps(summary)}", flush=True)

Path("runs/valencia/experiment_topology.json").write_text(json.dumps(out, indent=2))
