...
- [ ] **The notifications all arrive by email**, and a failed pull request run
  sends one every time, which will drown a personal inbox. Weighed on
  2026-09-13: turning Actions email off in GitHub's notification settings and
  keeping the web and mobile ones; or sending the repository's events to a
  channel of their own — GitHub's Slack app, or a Discord webhook, which is a
  URL and no account beyond the server. Decide before the first release, when
  the traffic starts.
