"""Stage 2+3 reproduction: incremental SfM + old-camera DLT pose.

Body copied verbatim from CV/DPO2/PoseEstimation/prueba.py -- the script that
produced the figures in the original presentation. Everything this file adds is
additive; no line of the original logic is altered:

  * Results are persisted. The original computed R_old_opt / t_old_opt / K_old
    -- the old camera pose, the whole point of the project -- and only plotted
    them, so nothing survived the run.
  * Output goes to $REPRO_OUT, never to CV/DPO2/PoseEstimation/results/. The
    original's commented-out save block called np.save on 3D_points.npy, which
    would clobber the one result artifact the baseline still has.
  * Figures are suppressed rather than shown; run with MPLBACKEND=Agg, which
    turns the original plt.show() calls into no-ops.

The body resolves its inputs with os.path.dirname(__file__), so __file__ is
pinned below to the original script's location. Without that, living in repro/
would send every '../RANSAC/results/...' lookup to the wrong directory.
"""
...
_sys.path.insert(0, str(_ORIGINAL))          # so `import sfm` finds the library
__file__ = str(_ORIGINAL / "prueba.py")      # so the body's relative paths resolve

_OUT = _Path(_os.environ.get("REPRO_OUT", "runs/scratch/sfm")).resolve()
...
# ============================
# PERSIST RESULTS  (added by the reproduction harness)
# ============================
import numpy as _np
...
# The old camera pose: the actual deliverable of the project, never saved by the original.
_np.savez(
    _OUT / "old_camera.npz",
...
(_OUT / "summary.json").write_text(_json.dumps(_summary, indent=2))
