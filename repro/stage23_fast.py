"""Stage 2+3, fast bundle adjustment. Same algorithm as repro/stage23_sfm.py.

Exactly three edits to the original prueba.py body, all mechanical:

  1. The multi-camera least_squares call uses fastba.MulticameraBA.residual
     instead of sfm.resBundleProjection_multicamera, plus `jac_sparsity` and
     `tr_solver='lsmr'`. fastba's residual is verified equal to the original to
     ~1e-11 relative, and Rodrigues matches expm(crossMatrix(.)) to 9e-16.
  2. AVAILABLE_IMAGES is read from $REPRO_IMAGES so the camera set is not baked
     into the source.
  3. Results are persisted to $REPRO_OUT (the original saved nothing).

Everything else, including the first-pair BA and the old-camera DLT, is
untouched, so any difference against runs/baseline-repro-01 comes from the
bundle adjustment alone.
"""
...
_ORIGINAL = _Path(__file__).resolve().parent.parent / "CV" / "DPO2" / "PoseEstimation"
_sys.path.insert(0, str(_ORIGINAL))
_sys.path.insert(0, str(_Path(__file__).resolve().parent))
__file__ = str(_ORIGINAL / "prueba.py")   # so the body's relative paths resolve

import fastba  # noqa: E402

_OUT = _Path(_os.environ.get("REPRO_OUT", "runs/scratch/sfm-fast")).resolve()
...
AVAILABLE_IMAGES = _os.environ.get(
    'REPRO_IMAGES', 'Img15_Try3_12M,Img23_Try1_12M').split(',')
...
for pnp_cam_idx in range(len(AVAILABLE_IMAGES)):
    ...
    start_time = time.time()
    fastba.reset_stats()
    # Dump the real BA inputs once, so the original residual can be benchmarked
    # on the actual problem rather than a synthetic stand-in.
    _dump = _OUT / f"ba_inputs_cam{pnp_camera_id}.npz"
    if not _dump.exists():
        import pickle as _pk
        with open(_OUT / f"ba_inputs_cam{pnp_camera_id}.pkl", "wb") as _f:
            _pk.dump({"point3D_map": point3D_map, "keypoints_2D": keypoints_2D,
                      "K": K_camera, "n_cameras": n_cameras,
                      "op_vector": op_vector}, _f)
    _ba = fastba.MulticameraBA(point3D_map, keypoints_2D, K_camera, n_cameras)
    _S = _ba.sparsity(op_vector.size)
    print(f"[fast] sparsity {_S.shape}, {_S.nnz:,} nnz "
          f"({100*_S.nnz/(_S.shape[0]*_S.shape[1]):.2f}% dense)")
    optimized_result = least_squares(
        _ba.residual,
        op_vector,
        method='trf',
        jac_sparsity=_S,      # group columns instead of one FD eval per parameter
        tr_solver='lsmr',     # sparse trust-region solve; 'exact' would SVD the full J
        verbose=2
    )
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Tiempo de optimización Ref-Cam{pnp_camera_id}: {elapsed_time:.6f} segundos")
    fastba.report(f"BA Ref-Cam{pnp_camera_id}", elapsed_time)
    print(f"[timer]   nfev={optimized_result.nfev} njev={optimized_result.njev}")
...
_np.savez(_OUT / "rotations.npz", *R_list)
_np.savez(_OUT / "translations.npz", *t_list)
_np.save(_OUT / "3D_points.npy", current_3DPoints_opt)
_np.savez(_OUT / "old_camera.npz",
          K_old=K_old, R_old=R_old, t_old=t_old,
          R_old_opt=R_old_opt, t_old_opt=t_old_opt,
          P_old=P_old, T_CamOld_opt=T_CamOld_opt)

(_OUT / "summary.json").write_text(_json.dumps({
    "n_points_final": int(current_3DPoints_opt.shape[0]),
    "n_cameras": int(len(R_list)) + 1,
    "reference_image": REFERENCE_IMAGE,
    "first_image": FIRST_IMAGE,
    "available_images": list(AVAILABLE_IMAGES),
    "K_old": _np.asarray(K_old).tolist(),
    "R_old_opt": _np.asarray(R_old_opt).tolist(),
    "t_old_opt": _np.asarray(t_old_opt).ravel().tolist(),
}, indent=2))
print(f"\n[repro] artifacts written to {_OUT}")
